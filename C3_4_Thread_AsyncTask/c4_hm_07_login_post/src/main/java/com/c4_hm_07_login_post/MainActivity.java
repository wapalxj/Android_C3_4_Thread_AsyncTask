package com.c4_hm_07_login_post;
...
//                    newPath = path+"number="+ URLEncoder.encode(number,"UTF-8")+"&pwd="+URLEncoder.encode(pwd,"UTF-8");
                    ...
                    //POST设置
                    String user="number="+ URLEncoder.encode(number,"UTF-8")+"&pwd="+URLEncoder.encode(pwd,"UTF-8");
                    conn.setRequestProperty("Content-Type","application/x-www-form-urlencoded");
                    conn.setRequestProperty("Content-Length",user.length()+"");
                    //向服务器发送登录参数
                    //number=252198&pwd=123456
                    conn.setDoOutput(true);
                    conn.getOutputStream().write(user.getBytes());
